function defaultSubmit() {
    ...
    if (reg.test(inputURL)) {
            ...
            grecaptcha.execute('6LclgSoaAAAAAEoDmmNxBurh9hdLoEKqni_iwO3k', {action: 'submit'}).then(function (token) {
                ...
                $.post("/API/api.php", {
                    ...
                    function (data, status) {
                        var txt1;
                        if (data.status == 200) {
                            txt1 = '<div class="alert alert-success fade in"><a href="#" class="close" data-dismiss="alert">&times;</a><strong>' + data.code + '</strong></div>';
                        } else {
        ...
        setTimeout(function (){$("#default-btn").removeAttr("disabled")},3000);
    } else {
